...
def Twitter_Crawler(driver, Keyword_Path, Stop_num, kw_start_point=0, save_path=None, start_date=None, end_date=None,
    ...
    :return:
    '''
    df = pd.read_csv(Keyword_Path, encoding='GB18030', header=0)
    page_index = 1
    search_end = False
                    ...
                    for divIndex, div in enumerate(divs):
                        ...
                        try:
                            ...
                            try:
                                ...
                                language = 'unknown'
                            # 写入dataSet
                            if ((language == limit_language or limit_language == 'en') and (
...
def SaveToCSV(Data_List, index, keyword_df, page_index, save_path):
    ...
    df_Sheet = pd.DataFrame(Data_List, columns=[
        'number' 'Name', 'User_name', 'Date', 'Content', 'Comments', 'Forward', 'Like', 'Language'])
    ...
    print(' the {} URL has accessed。'.format(page_index))
    try:
        print("%s tweet is crawled" % len(Data_List))
    except:
        print('意外')
